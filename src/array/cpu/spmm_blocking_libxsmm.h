...
inline libxsmm_meltwfunction_opreduce_vecs_idx SpMMCreateLibxsmmKernel(
  ...
  //opredop_flags = OP_ADD | OPORDER_VECIDX_VECIN | REDOP_SUM 
  libxsmm_meltwfunction_opreduce_vecs_idx kernel = nullptr;
  if (std::is_same<DType, float>::value) {
//    std::cout<<"DType is float!"<<std::endl;
//    std::cout<<"IdType size is "<<sizeof(IdType)<<std::endl;
...
void SpMMRedopCsrOpt(
...
#ifdef DEBUG
  std::chrono::high_resolution_clock::time_point startTick, endTick;
  ...
  const int nthreads = omp_get_max_threads(); //12 for bluestar
  const IdType M = csr.num_rows; //232965
  const IdType N = bcast.out_len; //128
  const IdType K = csr.num_cols; //232965
...
//                                                       nnz_prob * BLOCKING_HEURISTIC_PARAM)));
  int64_t k_blocking_param = std::stol(getenv("K_BLOCKING_PARAM"));
  int64_t m_blocking_param = std::stol(getenv("M_BLOCKING_PARAM"));

  IdType K_block_size = std::min((int64_t)K, (int64_t)(llc_size / (N * sizeof(DType) *
                                                       nnz_prob * k_blocking_param)));

  IdType M_block_size = M / (nthreads * m_blocking_param);
...
//  LOG(INFO) << "stage0 ticks = " << (double)std::chrono::duration_cast<std::chrono::microseconds>(endTick - startTick).count()/1000.0 <<" ms";
  startTick = std::chrono::high_resolution_clock::now();
#endif  // DEBUG
...
#ifdef DEBUG
  endTick = std::chrono::high_resolution_clock::now();
  auto blocking_ms = (double)std::chrono::duration_cast<std::chrono::microseconds>(endTick - startTick).count()/1000.0;
//  LOG(INFO) << "stage1 ticks = " << blocking_ms <<" ms";
  startTick = std::chrono::high_resolution_clock::now();
#endif  // DEBUG
...
#ifdef DEBUG
  endTick = std::chrono::high_resolution_clock::now();
//  LOG(INFO) << "stage2 ticks = " << (double)std::chrono::duration_cast<std::chrono::microseconds>(endTick - startTick).count()/1000.0 <<" ms";
  startTick = std::chrono::high_resolution_clock::now();
#endif  // DEBUG
...
#ifdef DEBUG
  endTick = std::chrono::high_resolution_clock::now();
  auto spmm_ms = (double)std::chrono::duration_cast<std::chrono::microseconds>(endTick - startTick).count()/1000.0;
//  LOG(INFO) << "stage3 ticks = " << spmm_ms <<" ms";
  startTick = std::chrono::high_resolution_clock::now();
#endif  // DEBUG
...
#ifdef DEBUG
  endTick = std::chrono::high_resolution_clock::now();
//  LOG(INFO) << "stage4 ticks = " << (double)std::chrono::duration_cast<std::chrono::microseconds>(endTick - startTick).count()/1000.0 <<" ms";
#endif  // DEBUG
  std::string file_name = "out.txt";
  std::ofstream f;
  f.open(file_name, std::fstream::out | std::fstream::app);
  f <<blocking_ms<<" " <<spmm_ms<<std::endl;
  f.close();
}
