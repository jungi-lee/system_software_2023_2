...
#               print("1st norm %f ms"%((time.time()-start)*1000))
...
#               print("gemm %f ms"%((time.time()-start)*1000))

                start = time.time()
                graph.update_all(aggregate_fn, fn.sum(msg='m', out='h'))
                rst = graph.dstdata['h']
#               print("spmm %f ms"%((time.time()-start)*1000))
            else:
...
#               print("spmm %f ms"%((time.time()-start)*1000))
 ...
 #              print("gemm %f ms"%((time.time()-start)*1000))
            ...
            if self._norm in ['right', 'both']:
                ...
                norm = th.reshape(norm, shp)
                rst = rst * norm
  #             print("2nd norm %f ms"%((time.time()-start)*1000))
  ...
  #         print("bias,act %f ms"%((time.time()-start)*1000))
